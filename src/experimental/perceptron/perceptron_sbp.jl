...
mutable struct FactorGraphTAP
    ...
    function FactorGraphTAP(ξ::Matrix{Int}, σ::Vector{Int})
        ...
        new(N, M, ξ, σ, zeros(N, M), zeros(M, N), zeros(N))
    end
end
...
function initrand!(g::FactorGraphTAP)
    g.m = rand([-1,1], g.N,g.M)
    g.m̂ = rand([-1,1], g.M, g.N)
end
    ...
    for i=1:N
        ...
        for a=1:M
         ...
         for a=1:M
             ...
             m̂cav = m̂[a, i]
             # @assert iseven(m̂cav) && isinteger(m̂cav)
            #  hcav = h[i] - (m̂cav <= -2 ? -s*m̂cav :
            #       m̂cav == 0 ? s /4 :
            #       0)
              hcav = h[i] - (m̂cav <= 0 ? s : 0)
              oldm = m[i, a]
              m[i, a] = sgn(hcav)
              Δ += abs(div(m[i,a] - oldm,2)) / N
          end
    end
...
mag(g::FactorGraphTAP, i::Integer) = sgn(g.h[i])
#
...
mags(g::FactorGraphTAP) = [sign(h) for h in g.h]
