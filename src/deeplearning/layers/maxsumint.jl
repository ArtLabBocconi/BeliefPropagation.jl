...
mutable struct MaxSumIntLayer <: AbstractLayer
    ...
    allmcav::IVecVecVec
    allmycav::IVecVecVec
    allmhcavtoy::IVecVecVec
    allmhcavtow::IVecVecVec
...
function MaxSumIntLayer(K::Int, N::Int, M::Int; βms=1., rms=1.)
    # for variables W
    allm = [zeros(N) for i=1:K]
    allh = [zeros(N) for i=1:K]


    allmcav = [[zeros(Int,N) for i=1:M] for i=1:K]
    allmycav = [[zeros(Int,N) for i=1:K] for i=1:M]
    allmhcavtoy = [[zeros(Int,K) for i=1:N] for i=1:M]
    allmhcavtow = [[zeros(Int,M) for i=1:N] for i=1:K]
    # for variables Y
    ...
    return MaxSumIntLayer(-1, K, N, M, allm, allmy, allmh
...
function updateVarW!{L <: Union{MaxSumIntLayer}}(layer::L, k::Int, r::Float64=0.)
...
function updateVarY!{L <: Union{MaxSumIntLayer}}(layer::L, a::Int, ry::Float64=0.)
...
function updateFact!(layer::MaxSumIntLayer, k::Int)
    ...
    for a=1:M
        ...
        mhw = allmhcavtow[k]
        mhy = allmhcavtoy[a]
        ϕyy = atanh(2pdtop[a]-1) / βms
        !isfinite(ϕyy) && (ϕyy = sign(ϕyy) * ∞)
        ϕy = round(Int, ϕyy)
...
function update!{L <: Union{MaxSumIntLayer}}(layer::L, r::Float64, ry::Float64)
...
function initrand!{L <: Union{MaxSumIntLayer}}(layer::L)
...
function fixY!{L <: Union{MaxSumIntLayer}}(layer::L, ξ::Matrix)
    @extract layer K N M allm allmy allmh allpu allpd  top_allpd
